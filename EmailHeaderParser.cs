...
namespace EmailHeaderParser
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var container = blobClient.GetContainerReference("emails");
            container.CreateIfNotExists(BlobContainerPublicAccessType.Blob);

            //Downlaod all emails with .eml extension stored in Azure Blob one by one by passing the email file names written in txt file. Store them in .txt format
            string[] lines = System.IO.File.ReadAllLines(@"C:\Downloads\EmailHeaderParser\DownloadFromBlob.txt");
            foreach (string line in lines)
            {
                Console.WriteLine("\t" + line);
                CloudBlockBlob blockBlob = container.GetBlockBlobReference("Inbox\\" + line + ".eml");
            ...
            // Email will be parsed based on your requirements
